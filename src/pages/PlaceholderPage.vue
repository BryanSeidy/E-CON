...
<template>
  <section class="space-y-6">
    <div class="rounded-2xl border border-border bg-white p-6 shadow-sm">
      <p class="text-sm font-semibold uppercase tracking-wide text-slate-500">Sprint Front 1</p>
      <h2 class="mt-2 text-3xl font-bold tracking-tight">{{ route.meta.label }}</h2>
      <p class="mt-3 max-w-3xl text-slate-600">Ecran connecté aux clients API générés depuis l’OpenAPI backend. Les sections sans endpoint backend exposé affichent une limitation explicite.</p>
    </div>

    <div v-if="activeView === 'profile' || activeView === 'university-students'" class="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-amber-900">
      <div class="flex gap-3"><AlertCircle class="mt-0.5 h-5 w-5 shrink-0"/><div><h3 class="font-semibold">Endpoint backend non exposé</h3><p class="mt-1 text-sm">La documentation OpenAPI disponible ne contient pas d’endpoint Accounts/Profile/Students. Cette interface n’invente donc ni modèle, ni formulaire d’édition, ni données mockées.</p><p v-if="activeView === 'profile'" class="mt-4 text-sm"><span class="font-medium">Session JWT locale :</span> {{ auth.user?.email || auth.user?.id || 'Utilisateur authentifié' }}</p></div></div>
    </div>
