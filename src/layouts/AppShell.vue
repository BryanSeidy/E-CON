...
<template>
  <div class="min-h-screen bg-slate-50 text-ink">
    ...
    >
      <div class="text-xl font-bold tracking-tight">E-CON</div>
      <p class="mt-1 text-sm text-slate-500">Gestion des stages</p>
      <nav class="mt-8 space-y-1">
        <RouterLink
          v-for="[href, label, Icon] in nav"
          :key="href as string"
          :to="href as string"
          ...
          ><component :is="Icon" class="h-4 w-4" />{{ label }}</RouterLink
        >
      </nav>
    </aside>
    <main class="pl-72">
      ...
      >
        ...
        <div class="flex items-center gap-3">
          ...
          ><button
            ...
            @click="logout"
          >
            <LogOut class="mr-2 inline h-4 w-4" />Déconnexion
          </button>
        </div>
      </header>
      <section class="p-8"><RouterView /></section>
    </main>
  </div>
</template>
