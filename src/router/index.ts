...
export const router = createRouter({
  history: createWebHistory(), scrollBehavior(_to, _from, savedPosition) { return savedPosition ?? { top: 0 } }, routes: [
    { path: '/', name: 'landing', component: Landing },
    { path: '/login', name: 'login', component: Login, meta: { guest: true } },
    {
      path: '/app', component: Shell, meta: { requiresAuth: true }, children: [
...
router.beforeEach((to) => { const auth = useAuthStore(); if (to.meta.requiresAuth && !auth.isAuthenticated) return { name: 'login', query: { redirect: to.fullPath } }; if (to.name === 'login' && auth.isAuthenticated) return { name: 'dashboard' }; if (to.name === 'landing' && auth.isAuthenticated) return { name: 'dashboard' }; if (!canAccess(to)) return { name: 'dashboard' } })
