...
function canAccess(to) { const auth = useAuthStore(); const meta = to.meta; if (!meta.roles?.length)
    return true; return Boolean(auth.role && meta.roles.includes(auth.role)); }
router.beforeEach((to) => { const auth = useAuthStore(); if (to.meta.requiresAuth && !auth.isAuthenticated)
    return { name: 'login', query: { redirect: to.fullPath } }; if (to.name === 'login' && auth.isAuthenticated)
    return { name: 'dashboard' }; if (!canAccess(to))
